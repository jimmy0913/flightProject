...
define(['jQuery', 'Base'], function ($, base) {
        ...
        $.api('get', 'get_airlines_by_supplier_and_date', {
            ...
            if (ret.status.code == 200 && ret.status.message == 'success' && ret.result.list.length > 0) {
                ...
                $.each(ret.result.list, function (i, d) {
                    ohtml += '<tr><td width="90px" >' + (i + 1 + limit * (current_page - 1)) + '</td><td width="120px">' + (d.supplier_id || '--') + '</td><td>' + d.object_id + '</td><td width="120px">' + (d.airline || '--') + '</td><td width="120px">' + d.current_markup + '</td><td width="100px">' + d.query_count + '</td><td width="100px">' + d.book_count + '</td><td width="120px">' + d.convert_rate + '</td><td width="120px">' + d.profit + '</td><td width="120px">' + d.profit_rate + '</td><td width="150px"><a class=\'td_a\' href="/airline/history/' + d.object_id + '?supplier_id=' + (d.supplier_id || '') + '&airline=' + (d.airline || '') + '&display_format=' + display_format + '&time=' + d.time + '">\u5386\u53F2\u52A0\u4EF7</a>\n                    </td></tr>';
                });
            } else {
    ...
    page_list = '', current_page = 1, total_page = null, limit = 15, time = $('#time').val();
